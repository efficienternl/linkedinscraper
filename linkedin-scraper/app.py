...
# TAB 6: Apify Search
with tabs[5]:
    st.header("🔍 Zoeken via Apify")
    st.info("Zoek op LinkedIn via Apify (jouw account loopt geen risico)")

    col1, col2 = st.columns(2)
    with col1:
        search_keywords = st.text_input("Zoekterm", placeholder="e.g. 'transport'")
    ...
    with col2:
        ...
        search_limit = st.number_input("Max. profielen", 10, 500, 100)
    ...
    if st.button("🔍 Zoeken & Scrapen", use_container_width=True, key="search_btn"):
        if not search_keywords:
            ...
            cmd = f'python cli.py search --keywords "{search_keywords}" --limit {search_limit} --out {search_output}'
...
### 📊 Output
Alle resultaten worden als JSONL opgeslagen (één profiel per regel in JSON format).
""")
